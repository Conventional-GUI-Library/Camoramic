...
CFLAGS = -Wno-deprecated-declarations $(shell $(PKGCONFIG) --cflags gtk+-3.0 gthread-2.0 gstreamer-video-1.0 libcanberra alsa)
LIBS = $(shell $(PKGCONFIG) --libs gtk+-3.0 gthread-2.0 gstreamer-video-1.0 libcanberra alsa) -lm
PACKAGE = camoramic

all: ${PACKAGE} po/tr_TR/$(PACKAGE).mo  po/it_IT/$(PACKAGE).mo  po/de_DE/$(PACKAGE).mo
...
po/de_DE/$(PACKAGE).mo: po/de_DE/$(PACKAGE).po
	msgfmt $(^) --output-file=$@ 
...
uninstall:
	...
	rm -f /usr/share/locale/de_DE/LC_MESSAGES/$(PACKAGE).mo
...
install: all
	...
	install -Dm0644 po/de_DE/$(PACKAGE).mo /usr/share/locale/de_DE/LC_MESSAGES/$(PACKAGE).mo
